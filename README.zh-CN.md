...
| X | 扩展，被动拦截页面自己发出的请求 | 否，单一书签列表 |
| GitHub | FavHub 直接调用公开 starred API | 否，单一 star 列表 |

X 是例外，因为它的 GraphQL 接口靠请求头鉴权。扩展自己一个请求都不发，而是挂住页面自己的
fetch 并记录响应体——再靠**自己滚动书签列表**促使页面去发这些请求。"被动"说的是请求，不是
滚动：你不需要滚，也不需要让那个标签页保持在前台。正因如此滚动是瞬时而非平滑的——平滑滚动
依赖的动画帧会被 Chrome 在后台标签页里暂停。
